 ...
 *
 *  Code slightly modified by Matt Smith, for use in fbreak.
 *
...
main(int argc, char **argv)
{
	...
	if(argc == 0)
		...
		strcpy(hexdigestline, sum(fd, argv[c]));
		printf("%s", hexdigestline);
		fclose(fd);
	}
}
*/
...
sum(FILE *fd, char *name)
{
	...
	md5(buf, n, digest, s);
	for(i=0;i<16;i++) sprintf(hexdigestline, "%s%.2X", hexdigestline, digest[i]);
	if(name)
		sprintf(hexdigestline, "%s\t%s", hexdigestline, name);
	strcat(hexdigestline, "\n");
	free(buf);

	return hexdigestline;
}
